...
export const ChatHeader = styled.header`
    ...
    .user-details {
      ...
      .header-info {
          ...
          h3 { color: var(--text-main); font-size: var(--text-base); font-weight: 600; margin-bottom: 2px; letter-spacing: 0.2px;}
          .presence-info {
              ...
              span { font-size: var(--text-xs); color: var(--text-dim); transition: color 0.3s; &.online { color: #10b981; } }
          }
          .chat-bio { font-size: var(--text-xs); color: var(--text-dim); display: flex; align-items: center; gap: 0.4rem; cursor: help; }
      }
    }
    
    .admin-controls {
        display: flex; align-items: center; gap: 1rem;
        .chat-search-input { background: var(--input-bg); color: var(--text-main); border: 1px solid var(--glass-border); padding: 0.5rem 1rem; border-radius: 2rem; outline: none; font-size: var(--text-sm); animation: ${springyPop} 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); transition: all 0.2s; &:focus { border-color: var(--adaptive-accent); box-shadow: 0 0 10px rgba(78, 14, 255, 0.2); } }
        .huddle-btn { background: var(--adaptive-accent); color: white; border: none; padding: 0.5rem 1rem; border-radius: 2rem; cursor: pointer; display: flex; align-items: center; gap: 0.5rem; font-weight: 600; font-size: var(--text-sm); transition: all 0.2s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.2); &:hover { transform: translateY(-2px); filter: brightness(1.1); } }
...
export const MessagesArea = styled.div`
    ...
    .loading-older { text-align: center; padding: 1rem; color: var(--adaptive-accent); font-size: var(--text-sm); font-weight: 500; opacity: 0.8; }
    ...
    .skeleton-anim { background: linear-gradient(90deg, var(--input-bg) 25%, var(--bg-panel) 50%, var(--input-bg) 75%); background-size: 200% 100%; animation: ${shimmer} 2s infinite ease-in-out; }

    .date-separator {
        display: flex; justify-content: center; align-items: center; margin: 1.5rem 0; position: relative;
        span { 
            background: var(--glass-noise), var(--bg-panel); border: 1px solid var(--glass-border); 
            padding: 0.4rem 1.2rem; border-radius: 2rem; font-size: var(--text-xs); font-weight: 500;
            color: var(--text-dim); z-index: 1; backdrop-filter: blur(8px); 
            box-shadow: 0 4px 15px rgba(0,0,0,0.05);
        }
        &::after { content: ""; position: absolute; width: 100%; height: 1px; background: linear-gradient(90deg, transparent, var(--glass-border), transparent); top: 50%; z-index: 0; }
    }
    ...
    .message {
      ...
      .content {
        max-width: 65%; word-wrap: break-word; word-break: break-word; 
        padding: 0.9rem 1.2rem; border-radius: 1.2rem;
        color: var(--text-main); font-size: var(--text-base); line-height: 1.45; display: flex; flex-direction: column;
        ...
        .message-actions {
            ...
            background: var(--glass-noise), var(--bg-panel); padding: 0.4rem 0.6rem; border-radius: 2rem;
            ...
            left: 100%; margin-left: 8px; transform: translateX(-10px) scale(0.95);
            
            button, .reaction-trigger {
                background: none; border: none; color: var(--text-dim); cursor: pointer;
                display: flex; align-items: center; justify-content: center;
                transition: 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275); font-size: var(--text-base); &:hover { color: var(--text-main); transform: scale(1.15); }
            }
            ...
            .reaction-menu {
                ...
                .reaction-emoji-btn { cursor: pointer; transition: 0.2s; font-size: var(--text-xl); display: inline-block; &:hover { transform: scale(1.4) translateY(-4px); } }
            }
        }
        ...
        .reactions-display {
            position: absolute; bottom: -14px; right: 14px; display: flex; flex-direction: row-reverse; gap: -6px; z-index: 3;
            
            .reaction-pill {
                background: var(--glass-noise), var(--bg-panel); padding: 0.15rem 0.4rem; border-radius: 1rem;
                font-size: var(--text-xs); border: 1px solid var(--glass-border);
        ...
        .sender-name { font-size: var(--text-xs); color: var(--adaptive-accent); font-weight: 700; margin-bottom: 6px; text-transform: capitalize; letter-spacing: 0.3px;}
        .deleted-text { font-style: italic; color: var(--text-dim); font-size: var(--text-sm); }
        .edited-tag { font-size: 0.65rem; opacity: 0.5; margin-left: 6px; font-style: italic; }
        .forwarded-tag { font-size: var(--text-xs); color: var(--text-dim); margin-bottom: 0.6rem; font-style: italic; display: flex; align-items: center; gap: 0.4rem; font-weight: 500;}
        
        .view-once-btn { background: linear-gradient(135deg, #ff0055, #ff5500); color: white; border: none; padding: 0.8rem 1.5rem; border-radius: 0.8rem; cursor: pointer; font-weight: 600; font-size: var(--text-sm); display: flex; align-items: center; gap: 0.6rem; box-shadow: 0 4px 15px rgba(255,0,85,0.3); transition: 0.2s; &:hover { filter: brightness(1.1); transform: translateY(-2px); box-shadow: 0 6px 20px rgba(255,0,85,0.5);} }
        ...
        .link-preview {
            ...
            .preview-card {
                ...
                .preview-info { padding: 1rem; h4 { margin: 0; font-size: var(--text-sm); font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } p { margin: 6px 0 0; font-size: var(--text-xs); color: var(--text-dim); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4; } }
            }
        }

        .poll-container {
            background: var(--input-bg); padding: 1.2rem; border-radius: 1rem; border: 1px solid var(--glass-border); min-width: 260px; margin: 6px 0;
            h4 { margin: 0 0 1rem 0; color: var(--text-main); display: flex; align-items: center; gap: 0.6rem; font-size: var(--text-base); font-weight: 600;}
            .poll-option {
                ...
                .opt-text, .opt-percent { position: relative; z-index: 1; font-size: var(--text-sm); font-weight: 500; }
            }
        }

        .quoted-message {
            background: var(--input-bg); border-left: 4px solid var(--adaptive-accent); padding: 0.6rem 0.8rem; border-radius: 0.4rem 0.8rem 0.8rem 0.4rem; font-size: var(--text-sm); margin-bottom: 0.8rem; color: var(--text-dim); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; cursor: pointer; transition: all 0.2s;
            &:hover { filter: brightness(0.9); }
            span { font-weight: 600; color: var(--adaptive-accent); }
        }

        .code-snippet { background: var(--bg-panel); padding: 1rem; border-radius: 0.8rem; overflow-x: auto; font-family: 'JetBrains Mono', 'Fira Code', monospace; border: 1px solid var(--glass-border); margin: 0.6rem 0; box-shadow: inset 0 2px 10px rgba(0,0,0,0.05); code { white-space: pre-wrap; word-break: break-all; font-size: var(--text-sm); line-height: 1.5; } }
        ...
        .msg-file-link { display: flex; align-items: center; gap: 0.8rem; background: var(--input-bg); padding: 0.8rem 1.2rem; border-radius: 0.8rem; color: var(--text-main); text-decoration: none; margin-top: 6px; font-weight: 600; font-size: var(--text-sm); transition: all 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275); border: 1px solid var(--glass-border); &:hover { filter: brightness(0.9); transform: translateY(-2px); box-shadow: 0 6px 15px rgba(0,0,0,0.1); } }

        .meta {
            display: flex; justify-content: flex-end; align-items: center;
            gap: 6px; font-size: 0.65rem; opacity: 0.8; margin-top: 8px; font-weight: 500; color: inherit;
            .timer-icon { color: #ff5500; font-size: var(--text-xs); animation: ${pulse} 2s infinite; }
            
            .read-status { font-weight: 700; font-size: var(--text-sm); display: flex; align-items: center; transition: 0.2s; }
            .read-status-wrapper { display: flex; align-items: center; gap: 4px; border-radius: 12px; padding: 2px 6px; transition: 0.2s; &.has-avatars { background: var(--input-bg); } &:hover.has-avatars { background: var(--glass-border); } }
            .reader-avatars { display: flex; align-items: center; margin-right: 2px; }
            .tiny-avatar { width: 16px; height: 16px; border-radius: 50%; border: 1px solid var(--msg-sent); margin-left: -6px; background: var(--bg-panel); object-fit: cover; &:first-child { margin-left: 0; } }
    ...
    .sended {
      ...
      .content {
        background: var(--glass-noise), var(--msg-sent);
        color: white; border-bottom-right-radius: 0.3rem; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
        &::before { right: -6px; background: var(--msg-sent); clip-path: polygon(0 0, 0% 100%, 100% 100%); }
        .message-actions { right: 100%; left: auto; margin-right: 8px; margin-left: 0; transform: translateX(10px) scale(0.95); } 
...
export const DropOverlay = styled.div`
    ...
    .overlay-content { 
        ...
        background: var(--glass-noise), var(--bg-panel); padding: 3rem 4rem; border-radius: 2rem; box-shadow: var(--glass-shadow);
        h2 { margin: 1rem 0; font-weight: 700; font-size: var(--text-xl); color: var(--text-main); } 
        p { color: var(--text-dim); font-size: var(--text-base); }
    }
`;

export const ScrollButton = styled.button`
    position: absolute; bottom: 90px; right: 30px; width: 48px; height: 48px;
    border-radius: 50%; background: var(--adaptive-accent); 
    color: white; border: none; cursor: pointer; box-shadow: 0 6px 20px rgba(0,0,0,0.2); 
    display: flex; justify-content: center; align-items: center; z-index: 10;
    transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); font-size: var(--text-lg);
    ...
    .unread-badge {
        ...
        font-size: var(--text-xs); font-weight: 700; min-width: 24px; height: 24px; display: flex;
...
export const Lightbox = styled.div`
    ...
    .close-btn { position: absolute; top: 30px; right: 30px; background: var(--input-bg); border-radius: 50%; width: 50px; height: 50px; display: flex; justify-content: center; align-items: center; border: 1px solid var(--glass-border); color: var(--text-main); font-size: var(--text-xl); cursor: pointer; transition: 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275); backdrop-filter: blur(4px); &:hover { transform: scale(1.1); background: rgba(255,78,78,0.8); color: white; border-color: transparent;} }
    
    .receipt-modal {
        background: var(--glass-noise), var(--bg-panel); padding: 2.5rem; border-radius: 24px; width: 420px;
        border: 1px solid var(--glass-border); box-shadow: var(--glass-shadow); position: relative; color: var(--text-main); 
        
        .close-btn-small { position: absolute; top: 20px; right: 20px; background: var(--input-bg); border-radius: 50%; width: 32px; height: 32px; display: flex; justify-content: center; align-items: center; border: none; color: var(--text-dim); cursor: pointer; font-size: var(--text-base); transition: 0.2s; &:hover { color: #ff4e4e; transform: scale(1.1); } }
        h3 { margin-bottom: 1.2rem; border-bottom: 1px solid var(--glass-border); padding-bottom: 1rem; font-weight: 700; font-size: var(--text-lg); letter-spacing: 0.5px; }
        .msg-preview { background: var(--input-bg); padding: 1.2rem; border-radius: 12px; font-style: italic; color: var(--text-dim); font-size: var(--text-sm); margin-bottom: 2rem; border-left: 4px solid #34B7F1; }
        ...
        .readers-list {
            ...
            h4 { color: #34B7F1; display: flex; align-items: center; gap: 0.6rem; margin-bottom: 1.2rem; font-weight: 600; font-size: var(--text-sm); text-transform: uppercase; letter-spacing: 0.5px; }
            .reader-item { 
                ...
                .reader-info { display: flex; align-items: center; gap: 12px; flex: 1; .reader-avatar-img { width: 36px; height: 36px; border-radius: 50%; background: var(--bg-panel); border: 2px solid rgba(52, 183, 241, 0.5); object-fit: cover; box-shadow: 0 4px 10px rgba(0,0,0,0.1); } .reader-name { font-weight: 600; font-size: var(--text-sm); color: var(--text-main); } }
                .reader-time { font-size: var(--text-xs); color: var(--text-dim); font-weight: 500; }
            }
        }
    }
`;
...
export const SideInfoPanel = styled.div`
  ...
  .panel-header { display: flex; justify-content: space-between; align-items: center; padding: 1.5rem; border-bottom: 1px solid var(--glass-border); background: transparent; }
  .tabs { display: flex; border-bottom: 1px solid var(--glass-border); background: transparent; button { flex: 1; background: none; border: none; padding: 1rem; color: var(--text-dim); font-weight: 600; cursor: pointer; transition: all 0.2s ease; font-size: var(--text-xs); text-transform: uppercase; letter-spacing: 0.5px; &.active { color: var(--text-main); border-bottom: 3px solid var(--adaptive-accent); background: var(--input-bg); } &:hover:not(.active) { color: var(--text-main); background: var(--input-bg); } } }
  ...
  .panel-content {
    ...
    .loader { display: flex; justify-content: center; align-items: center; height: 100px; color: var(--adaptive-accent); font-size: var(--text-xl); }
    .empty-state { color: var(--text-dim); text-align: center; margin-top: 3rem; font-style: italic; font-size: var(--text-sm); }
    .tab-search { display: flex; align-items: center; background: var(--input-bg); border: 1px solid var(--glass-border); border-radius: 12px; padding: 0.6rem 1rem; margin-bottom: 1.5rem; .icon { color: var(--text-dim); margin-right: 10px; } input { background: none; border: none; color: var(--text-main); outline: none; width: 100%; font-size: var(--text-sm); } }
  }
`;