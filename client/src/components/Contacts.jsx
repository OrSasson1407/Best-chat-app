...
import { FaUserFriends, FaPlus, FaSearch, FaUserEdit, FaCog } from "react-icons/fa";
...
export default function Contacts({ 
  ...
  handleLogout, 
  currentUser,
  theme,        // NEW: Passed down from Chat.jsx
  setTheme,     // NEW: Passed down from Chat.jsx
  isCompact,    // NEW: Passed down from Chat.jsx
  setIsCompact  // NEW: Passed down from Chat.jsx
}) {
  ...
  const [isLoading, setIsLoading] = useState(true); // NEW: Loading state for Skeletons
    ...
    <>
      {currentUserName && (
        ...
        <Container $isCompact={isCompact} $themeType={theme}>
                ...
                <>
                    ...
                    {filteredGroups.map((group, index) => (
                        <div key={group._id} className={`contact ${index === currentSelected ? "selected" : ""}`} onClick={() => changeCurrentChat(index, group, true)}>
                            {!isCompact && <div className="avatar group-avatar">#</div>}
          ...
          <div className="current-user">
             <div className="user-info">
                 ...
                 <div className="actions">
                     {/* Updated Icon to a Settings Gear since it now holds UI options too */}
          ...
          {/* UPDATED: Profile & Settings Modal */}
          {showProfileModal && (
              <Modal>
...
const Modal = styled.div`
    ...
    .modal-content {
        ...
        .divider { border-color: rgba(255,255,255,0.1); margin: 1rem 0; }
        .settings-row { display: flex; gap: 1rem; }
        
        select, input, textarea { width: 100%; padding: 0.8rem; margin-bottom: 0.5rem; background: rgba(255, 255, 255, 0.05); border: 1px solid #4e0eff; color: white; border-radius: 0.5rem; font-family: inherit; &:focus { outline: none; border-color: #9a86f3; } }
        
        .toggle-btn { padding: 0.8rem; background: rgba(255,255,255,0.05); color: white; border: 1px solid #4e0eff; border-radius: 0.5rem; cursor: pointer; width: 100%; transition: 0.2s;}
        .toggle-btn.active { background: #4e0eff; }
        
        textarea { resize: none; height: 80px; }
        .input-group { display: flex; flex-direction: column; flex: 1; label { color: #aaa; font-size: 0.8rem; margin-bottom: 0.3rem; } }
...
const Container = styled.div`
  ...
  .search-bar { 
      ...
      .search-icon { position: absolute; left: 1.5rem; color: #ccc; font-size: 0.9rem; } 
  ...
  .contacts {
    ...
    .skeleton-anim { background: #2a2a35; background-image: linear-gradient(to right, #2a2a35 0%, #3a3a45 20%, #2a2a35 40%, #2a2a35 100%); background-repeat: no-repeat; background-size: 800px 100%; animation: ${shimmer} 1.5s infinite linear forwards; }
    .skeleton-line { height: 12px; width: 100px; border-radius: 4px; margin-bottom: 5px; }
