...
export default function Login() {
  ...
  const validateForm = () => {
    if (!values.username || !values.password) { toast.error("Please fill in all fields."); return false; }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === "2fa_required") { setTwoFactorUserId(data.userId); setTwoFactorPending(true); return; }
          ...
          <form onSubmit={handleTwoFactorSubmit} noValidate>
            <div className="twofa-prompt">
              ...
              <input
                ...
                className="twofa-input" autoFocus disabled={isSubmitting}
              />
            </div>
            ...
            <p className="footer-link" style={{ marginTop: "1rem" }}>
              <button type="button"
                style={{ background: "none", border: "none", color: "var(--msg-sent)", cursor: "pointer", font: "inherit", fontSize: "var(--text-sm)" }}
                onClick={() => { setTwoFactorPending(false); setTwoFactorCode(""); }}
            ...
            <div className={`field-group ${focused === "username" ? "focused" : ""}`}>
              ...
              <div className="input-wrap">
                ...
                <input id="username" type="text" name="username" placeholder="Enter your username"
                  autoComplete="username" value={values.username} onChange={handleChange}
                  onFocus={() => setFocused("username")} onBlur={() => setFocused(null)} disabled={isSubmitting} />
              </div>
            </div>
...
const PageWrap = styled.div`
  ...
  .orb-1 { width:min(600px,70vw);height:min(600px,70vw);top:-15%;left:-12%;
            background:radial-gradient(circle,rgba(124,58,237,0.25),transparent 70%);filter:blur(70px); }
...
const Card = styled.div`
  ...
  padding: clamp(2rem,5vw,3rem); width: min(440px,95vw);
  ...
  .brand {
    ...
    .logo-badge { width:44px;height:44px;border-radius:14px;background:var(--aurora-gradient);
      display:flex;align-items:center;justify-content:center;
  ...
  .field-group {
    ...
    .input-wrap {
      ...
      background:var(--input-bg);border:1px solid var(--border-default);border-radius:12px;transition:all var(--duration-base);
      .field-icon { position:absolute;left:14px;color:var(--text-tertiary);flex-shrink:0;pointer-events:none; }
      input { width:100%;background:transparent;border:none;color:var(--text-primary);
        font-family:'Plus Jakarta Sans',sans-serif;font-size:var(--text-sm);padding:13px 44px 13px 42px;
        &::placeholder{color:var(--text-tertiary);}&:disabled{opacity:0.5;cursor:not-allowed;} }
      .toggle-pass { position:absolute;right:12px;background:none;border:none;color:var(--text-tertiary);
        cursor:pointer;padding:4px;border-radius:6px;display:flex;align-items:center;transition:color var(--duration-fast);
        &:hover{color:var(--text-primary);} }
    }
    &.focused .input-wrap {
      border-color:var(--msg-sent);background:rgba(124,58,237,0.06);box-shadow:0 0 0 3px rgba(124,58,237,0.12);
      .field-icon{color:var(--msg-sent);}
    }
  }

  .btn-submit {
    margin-top:0.5rem;width:100%;padding:14px;background:var(--aurora-gradient);
    ...
    transition:all var(--duration-base) var(--ease-out);box-shadow:0 6px 20px rgba(124,58,237,0.35);
    ...
    &:hover:not(:disabled)::before{opacity:1;}&:active{transform:translateY(0);}&:disabled{opacity:0.65;cursor:not-allowed;}
  ...
  .footer-link {
    margin-top:1.25rem;text-align:center;font-size:var(--text-sm);color:var(--text-secondary);
  ...
  .twofa-input {
    width:100%;padding:14px;text-align:center;font-size:2rem;letter-spacing:14px;
    font-family:monospace;font-weight:700;background:var(--input-bg);
    border:1.5px solid var(--border-default);border-radius:var(--radius-md);color:var(--text-primary);outline:none;
  ...
  .security-badge {
    ...
    margin-top:1.25rem;font-size:var(--text-2xs);color:var(--text-tertiary);font-weight:500;letter-spacing:0.3px;
  ...
  @media(max-width:480px){padding:1.75rem 1.5rem;border-radius:20px;}
`;
